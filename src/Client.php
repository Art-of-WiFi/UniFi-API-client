...
class Client
{
    ...
    /**
     * Set specific VLAN on devie port
     * 
     ...
     */
    public function set_port_vlan ( $device_mac, $port_idx, $vlan_id ) {

        $device = $this->list_devices($device_mac);
        ...
        foreach ($this->list_networkconf() as $key => $network) {
        ...
        if ( !$exist ) {
            array_push($device[0]->port_overrides, array(
        ...
        $res = $this->set_device_settings_base($device[0]->device_id, $payload);
